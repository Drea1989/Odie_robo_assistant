...
from odie.core.ActionModule import ActionModule, InvalidParameterException

logging.basicConfig()
logger = logging.getLogger("odie")


class Uri(ActionModule):
    ...
    def do_get(self):
        logger.debug(self.action_name + " do_get method called")
        r = requests.get(url=self.url, **self.parameters)
        self.post_processing_request(r)

    def do_post(self):
        logger.debug(self.action_name + " do_post method called")
        r = requests.post(url=self.url, **self.parameters)
        self.post_processing_request(r)

    def do_delete(self):
        logger.debug(self.action_name + " do_delete method called")
        r = requests.delete(url=self.url, **self.parameters)
        self.post_processing_request(r)

    def do_put(self):
        logger.debug(self.action_name + " do_put method called")
        r = requests.put(url=self.url, **self.parameters)
        self.post_processing_request(r)

    def do_head(self):
        logger.debug(self.action_name + " do_head method called")
        r = requests.head(url=self.url, **self.parameters)
        self.post_processing_request(r)

    def do_patch(self):
        logger.debug(self.action_name + " do_patch method called")
        r = requests.patch(url=self.url, **self.parameters)
        self.post_processing_request(r)

    def do_options(self):
        logger.debug(self.action_name + " do_options method called")
    ...
    def get_parameters(self):
        ...
        logger.debug(self.action_name + " parameters: %s" % returned_parameters)
    ...
    def post_processing_request(self, r):
        ...
        # we try to load into a json object the content. So Odie can use it to talk
        try:
            self.content = json.loads(self.content.decode())
        except ValueError:
            logger.debug(self.action_name + "cannot get a valid json from returned content")
            pass
        self.text = r.text
        self.response_header = r.headers

        logger.debug(self.action_name + " status_code: %s" % self.status_code)
        logger.debug(self.action_name + " content: %s" % self.content)
        logger.debug(self.action_name + " response_header: %s" % self.response_header)

    def _is_parameters_ok(self):
        """
        Check that all provided parameters in the actions are valid
