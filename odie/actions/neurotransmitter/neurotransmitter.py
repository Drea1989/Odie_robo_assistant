...
class Neurotransmitter(ActionModule):
    ...
    def callback(self, audio):
        """
        The callback used by the STT module to get the linked neuron
    ...
    def _is_parameters_ok(self):
        ...
        :return: true if the action is well configured, raise an exception otherwise

        .. raises:: MissingParameterException, InvalidParameterException
        """
        # with the action the user has the choice of a direct link that call another neuron,
        ...
        if self.from_answer_link is not None:
            if self.default is None:
                raise InvalidParameterException("default parameter is required and must contain a valid neuron name")
            for el in self.from_answer_link:
                if "neuron" not in el:
                    raise MissingParameterException("Links must contain a neuron name: %s" % el)
