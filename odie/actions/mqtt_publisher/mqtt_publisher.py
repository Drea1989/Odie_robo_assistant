...
class Mqtt_publisher(ActionModule):
    def __init__(self, **kwargs):
        super(Mqtt_publisher, self).__init__(**kwargs)

        logger.debug("[mqtt_publisher] action called with parameters: %s" % kwargs)
        ...
        self.client_id = kwargs.get('client_id', 'odie')
        ...
        if not self._is_parameters_ok():
            logger.debug("[mqtt_publisher] One or more invalid parameters, action will not be launched")
        else:
