...
class MqttClient(Thread):
    ...
    def on_message(self, client, userdata, msg):
        ...
        self.call_concerned_neurons(msg.topic, msg.payload)
    ...
    def call_concerned_neurons(self, topic_name, message):
        """
        Call neuron launcher class for each neuron concerned by the subscribed topic
        convert the message to json if needed before.
        The neuron is loaded with a parameter called "mqtt_subscriber_message" that can be used in actions
        ...
        # run each neuron
        for neuron in target_topic.neurons:
            logger.debug("[MqttClient] start neuron name %s" % neuron.name)
            overriding_parameter_dict = dict()
            overriding_parameter_dict["mqtt_subscriber_message"] = message
            NeuronLauncher.start_neuron_by_name(neuron.name,
                                                brain=self.brain,
