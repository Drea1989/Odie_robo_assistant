...
class ShellGui:
    def __init__(self, brain=None):
        ...
        self.d.set_background_title("odie shell UI")
    ...
    def show_main_menu(self):
        ...
        code, tag = self.d.menu("Test your odie settings from this menu",
                                choices=[("TTS", "Text to Speech"),
                                         ("STT", "Speech to text"),
                                         ("Neurons", "Run a Neuron")])
    ...
    def show_Neurons_test_menu(self):
        """
        Show a list of available Neuron in the brain to run it directly
        """

        # create a tuple for the list menu
        choices = list()
        x = 0
        for el in self.brain.Neurons:
            tup = (str(el.name), str(x))
            choices.append(tup)
            x += 1

        code, tag = self.d.menu("Select a Neuron to run",
                                choices=choices)

        if code == self.d.CANCEL:
            self.show_main_menu()
        if code == self.d.OK:
            logger.debug("Run Neuron from GUI: %s" % tag)
            NeuronLauncher.start_Neuron_by_name(tag, brain=self.brain)
            self.show_Neurons_test_menu()
