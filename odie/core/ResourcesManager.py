...
# LOCAL_TMP_FOLDER = "/tmp/odie/resources/"
TMP_GIT_FOLDER = "odie_new_module_temp_name"
...
class ResourcesManager(object):
    def __init__(self, **kwargs):
        ...
        self.tmp_path = tempfile.gettempdir() + "/odie/resources/" + TMP_GIT_FOLDER
        ...
        if self.is_repo_ok(dna_file_path=self.dna_file_path,
            ...
            if self.dna is not None:
                ...
                if self.is_settings_ok(resources=self.settings.resources, dna=self.dna):
                    ...
                    if self._check_supported_version(current_version=self.settings.odie_version,
                                                     supported_versions=self.dna.odie_supported_version):
    ...
    def is_settings_ok(resources, dna):
        ...
        if resources is None:
            ...
            if dna.module_type == "action" and resources.action_folder is None:
                message = "Resources folder for action installation not set in settings, cannot install."
                logger.debug(message)
                Utils.print_danger(message)
                settings_ok = False
            ...
            if dna.module_type == "tts" and resources.tts_folder is None:
                ...
                Utils.print_danger(message)
                settings_ok = False
            if dna.module_type == "wakeon" and resources.wakeon_folder is None:
                message = "Resources folder for wakeon installation not set in settings, cannot install."
                logger.debug(message)
                Utils.print_danger(message)
                settings_ok = False
    ...
    def _clone_repo(path, git_url):
        """
        Use git to clone locally the action in a temp folder
        :return:
        """
        # clone the repo
    ...
    def _rename_temp_folder(name, target_folder, tmp_path):
        ...
        new_absolute_action_path = target_folder + os.sep + name
        try:
            shutil.move(tmp_path, new_absolute_action_path)
            return new_absolute_action_path
        except shutil.Error:
    ...
    def run_ansible_playbook_module(install_file_path):
        """
        Run the install.yml file through an Ansible playbook using the dedicated action !

        :param install_file_path: the path of the Ansible playbook to run.
        :return:
        """
        logger.debug("[ResourcesManager] Run ansible playbook")
        Utils.print_info("Starting action installation")
        ...
        if not pswd or pswd == "":
            ...
            ansible_action_parameters = {
                "task_file": install_file_path,
                "sudo": True,
                "sudo_user": "root",
                "sudo_password": pswd
            }
            action = Action(name="ansible_playbook", parameters=ansible_action_parameters)
            ActionLauncher.start_action(action)
            return True

    @staticmethod
    def _check_supported_version(current_version, supported_versions):
        """
        The dna file contains supported Odie version for the module to install.
        Check if supported versions are match the current installed version. If not, ask the user to confirm the
        installation anyway
        :param current_version: current version installed of Odie. E.g 0.4.0
        ...
        """
        logger.debug("[ResourcesManager] Current installed version of Odie: %s" % str(current_version))
        ...
        if not supported_version_found:
            ...
            Utils.print_info("Current installed version of Odie: %s" % current_version)
            Utils.print_info("Module supported versions: %s" % str(supported_versions))
            Utils.print_warning("The action seems to be not supported by your current version of Odie")
