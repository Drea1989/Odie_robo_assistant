...
class ActionModule(object):
    """
    This Abstract Class is representing main Class for Action.
    Each Action must implement this Class.
    """
    def __init__(self, **kwargs):
        """
        Class used by action for talking
        ...
        # if the current call want to mute odie
        self.no_voice = kwargs.get('no_voice', False)
        # boolean to know id the neuron is waiting for an answer
    ...
    def say(self, message):
        ...
        if isinstance(message, list):
            logger.debug("[ActionModule] message is list")
            tts_message = random.choice(message)

        if isinstance(message, dict):
            logger.debug("[ActionModule] message is dict")
            tts_message = self._get_message_from_dict(message)

        if tts_message is not None:
            logger.debug("[ActionModule] tts_message to say: %s" % tts_message)
            self.tts_message = tts_message
            Utils.print_success(tts_message)

            # process the audio only if the no_voice flag is false
            if self.no_voice:
                logger.debug("[ActionModule] no_voice is True, Odie is muted")
            else:
                logger.debug("[ActionModule] no_voice is False, make Odie speaking")
                ...
                # Odie will talk, turn on the LED
                ...
                # Odie has finished to talk, turn off the LED
    ...
    def get_audio_from_stt(callback):
        ...
        ol.start()
        ol.join()
        # wait that the STT engine has finish his job (or the neurotransmitter action will be killed)
        if ol.stt_instance is not None:
            ol.stt_instance.join()

    def get_action_name(self):
        """
        Return the name of the action who call the mother class
        :return:
        """
    ...
    def _get_tts_object(tts_name=None, override_parameter=None, settings=None):
        ...
        if tts_object is None:
            raise TTSModuleNotFound("[ActionModule] The tts module name %s does not exist in settings file" % tts_name)

        if override_parameter is not None:  # the user want to override the default TTS configuration
            logger.debug("[ActionModule] args for TTS plugin before update: %s" % str(tts_object.parameters))
            for key, value in override_parameter.items():
                tts_object.parameters[key] = value
            logger.debug("[ActionModule] args for TTS plugin after update: %s" % str(tts_object.parameters))

        logger.debug("[ActionModule] TTS args: %s" % tts_object)
        return tts_object
