# build last version
# docker build --force-rm=true -t compile-snowboy-python35 -f compile_snowboy_python35.dockerfile .

# build specified version
# docker build --force-rm=true --build-arg SNOWBOY_VERSION=1.1.1 -t compile-snowboy-python35 -f compile_snowboy_python35.dockerfile .

# compile into local /tmp/snowboy
# docker run -it --rm --mount type=bind,source=/tmp/snowboy,target=/data compile-snowboy-python35
